...
typedef struct
{
     ...
     /*链表结构，线程池中所有等待任务*/
     CThread_worker *queue_head;    /* 任务队列头指针 */
...
static CThread_pool *pool = NULL;   /* 定义一个线程池 */
...
pool_add_worker (void *(*process) (void *arg), void *arg)
{
    ...
    /*
       将任务加入到等待队列中,如果队列中有其他任务，则将新任务加到队尾，否则为
       队首
    */
    ...
    if (member != NULL)
     ...
     assert (pool->queue_head != NULL); /* 如果线程池的任务队列为空 程序abort */
