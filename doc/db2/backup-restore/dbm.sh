...
#################################
# 此备份程序只针对已存在的数据库而言
# 1. 对于已经存在的数据库，是否会将 数据库目录 与 自动存储目录 修改为备份介质的 数据库目录 与 自动存储目录(*是)
# 2. 如果表空间容器变化了，要进行重定向恢复(*是)
# 3. 恢复到其他名字的数据库是否可以(*是) 
# 4. 对于compress压缩备份而言，64位机与32位机的备份介质之间不能够相互恢复(*在某些平台下可以相互回复)
...
function restore {
    ...
    auto='no';          # 数据库自动存储路径: 默认为 'no' 同库回复或者自动存储路径相同
    while getopts "s:p:v:t:r:f:a:" arg
    do
        case $arg in
            s)
                sdb=$OPTARG;
    ...
    if [ "x$redirect" == "xyes" ]; then
        if [ "x$forward" == "xno" ]; then
            if [ "x$auto" == "xno" ]; then
                ...
                db2 restore db $sdb incremental automatic from $path taken at $version on $auto into $tdb redirect generate script redirect.ddl without rolling forward;
                sta=$?;
            fi
        else
            if [ "x$auto" == "xno" ]; then
                ...
                db2 restore db $sdb incremental automatic from $path taken at $version on $auto into $tdb redirect generate script redirect.ddl;
                sta=$?;
            fi
        fi
        if [ "x$sta" == "x0" ]; then
...
";
        ...
        if [ "x$forward" == "xno" ]; then
            if [ "x$auto" == "xno" ]; then
                ...
                db2 restore db $sdb incremental automatic from $path taken at $version on $auto into $tdb without rolling forward;
                sta=$?;
            fi
        else
            if [ "x$auto" == "xno" ]; then
                ...
                db2 restore db $sdb incremental automatic from $path taken at $version on $auto into $tdb;
                sta=$?;
            fi
        fi
    fi
...
dbm.sh  backup                  (*备份操作)
        ...
        -t 'inc'                (备份方式. 'all'：全备; 'inc'：增量备份.                                     默认：'inc')
